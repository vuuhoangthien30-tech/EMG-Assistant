            ...
            <motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="w-full max-w-5xl h-[85vh] bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex">
              ...
              <div className="hidden sm:flex w-72 bg-[#F9FAFB] border-r border-[#E5E7EB] flex-col p-8">
                ...
                <div className="space-y-3">
                  <button onClick={() => handleSendMessage("Explain this like a teacher")} className="w-full flex items-center gap-3 p-4 rounded-2xl bg-white border border-transparent hover:border-blue-200 hover:shadow-md transition-all text-left text-sm font-medium group">
                    <BookOpen size={18} className="text-blue-500" />
                    <span>Ask like a teacher</span>
                  </button>
                  <button onClick={() => handleSendMessage("Summarize my tasks")} className="w-full flex items-center gap-3 p-4 rounded-2xl bg-white border border-transparent hover:border-blue-200 hover:shadow-md transition-all text-left text-sm font-medium group">
                    <ListChecks size={18} className="text-green-500" />
                    <span>Summarize tasks</span>
                  </button>
                  <button onClick={() => handleSendMessage("Give me STEM project ideas")} className="w-full flex items-center gap-3 p-4 rounded-2xl bg-white border border-transparent hover:border-blue-200 hover:shadow-md transition-all text-left text-sm font-medium group">
                    <Lightbulb size={18} className="text-yellow-500" />
                    <span>STEM Ideas</span>
                  </button>
                </div>

                <div className="mt-auto p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <p className="text-[10px] font-bold text-blue-600 uppercase mb-1">Current Focus</p>
                  <p className="text-xs font-bold text-blue-900">Week 28 Science</p>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col">
                <div className="p-6 border-b flex justify-between items-center bg-white">
                  <div className="flex items-center gap-3">
                    ...
                    <h3 className="font-bold">Assistant Online</h3>
                  </div>
                  <button onClick={() => setChatOpen(false)} className="p-2 hover:bg-gray-100 rounded-xl transition-colors text-gray-400"><ChevronRight size={24} className="rotate-90 sm:rotate-0" /></button>
                </div>

                <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-white">
                  {messages.map((msg, idx) => (
                    <div key={idx} className={cn("flex gap-4", msg.role === 'user' ? "flex-row-reverse" : "flex-row")}>
                      <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center shadow-sm", msg.role === 'user' ? "bg-white border" : "bg-[#0056B3] text-white")}>
                        {msg.role === 'user' ? 'ME' : <Sparkles size={18} />}
                      </div>
                      <div className={cn("max-w-[75%] p-5 rounded-2xl shadow-sm leading-relaxed", msg.role === 'user' ? "bg-blue-600 text-white rounded-tr-none" : "bg-gray-50 border text-gray-800 rounded-tl-none")}>
                ...
                <div className="p-8 bg-white border-t">
                  <div className="flex gap-4 bg-gray-50 p-2 rounded-[1.5rem] border focus-within:ring-2 focus-within:ring-blue-500 transition-all">
                    <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Type something..." className="flex-1 bg-transparent border-none px-4 py-2 outline-none text-sm" />
                    <button onClick={() => handleSendMessage()} disabled={isLoading} className="bg-[#0056B3] text-white p-3 rounded-2xl hover:opacity-90 transition-opacity disabled:opacity-50"><Send size={20} /></button>
                  </div>
                </div>
              </div>
